...
import { TreasuryActivity } from "../components/treasury/TreasuryActivity";
import { formatPeg } from "../components/treasury/format";
import { fetchActivity, fetchTreasuryOverview } from "../services/treasuryMockApi";
...
export default function ActivityPage() {
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetchActivity({ filter, signal: controller.signal })
      .then(setActivity)
      .catch((e) => {
        if (e?.name === "AbortError") return;
      ...
      <main className="mx-auto max-w-[1600px] space-y-8 px-6 py-10">
        <section className="space-y-6">
          ...
          {!activity ? (
            ...
            <TreasuryActivity data={activity} filter={filter} onFilterChange={setFilter} />
          )}
        </section>
      </main>
    </div>
