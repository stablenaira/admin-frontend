...
import { TreasuryMerchants } from "../components/treasury/TreasuryMerchants";
import { formatPeg } from "../components/treasury/format";
import { fetchMerchants, fetchTreasuryOverview } from "../services/treasuryMockApi";
...
export default function Merchants() {
  ...
  const [merchantsTimeframe, setMerchantsTimeframe] = useState("30D");
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetchMerchants({ timeframe: merchantsTimeframe, signal: controller.signal })
      .then(setMerchants)
      .catch((e) => {
        if (e?.name === "AbortError") return;
      ...
      <main className="mx-auto max-w-[1600px] space-y-8 px-6 py-10">
        <section className="space-y-6">
          ...
          {!merchants ? (
            ...
            <TreasuryMerchants data={merchants} timeframe={merchantsTimeframe} onTimeframeChange={setMerchantsTimeframe} />
          )}
        </section>
      </main>
    </div>
