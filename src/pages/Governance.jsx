...
import { TreasuryGovernance } from "../components/treasury/TreasuryGovernance";
import { formatPeg } from "../components/treasury/format";
import { fetchGovernance, fetchTreasuryOverview } from "../services/treasuryMockApi";
...
export default function Governance() {
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetchGovernance({ signal: controller.signal })
      .then(setGovernance)
      .catch((e) => {
        if (e?.name === "AbortError") return;
    ...
    <div className="text-white" style={{ fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-[1600px] space-y-8 px-6 py-10">
        <section className="space-y-6">
          ...
          {!governance ? <LoadingBlock label="Loading governance…" /> : <TreasuryGovernance data={governance} />}
        </section>
      </main>
    </div>
  );
