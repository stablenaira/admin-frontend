...
import { TreasuryPoolsSection } from "../components/treasury/TreasuryPoolsSection";
import { formatPeg } from "../components/treasury/format";
import { fetchPools, fetchTreasuryOverview } from "../services/treasuryMockApi";
...
export default function Pools() {
  ...
  useEffect(() => {
    const controller = new AbortController();
    fetchPools({ timeframe, signal: controller.signal })
      .then(setPools)
      .catch((e) => {
        if (e?.name === "AbortError") return;
    ...
    <div className="text-white" style={{ fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-[1600px] space-y-8 px-6 py-10">
        <section className="space-y-6">
          ...
          {!pools ? (
            <LoadingBlock label="Loading pools…" />
          ) : (
            <TreasuryPoolsSection data={pools} timeframe={timeframe} onTimeframeChange={setTimeframe} />
          )}
        </section>
      </main>
    </div>
